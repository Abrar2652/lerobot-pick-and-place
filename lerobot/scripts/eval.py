...
def rollout(
    ...
    while not np.all(done):
        ...
        try:
            ...
            print(f"XXXXXX------......******\nXXXXXXXX------------XXXXXXXXXXXXXX\nPhysics error encountered: {e}")
            reward = np.ones((env.num_envs,)) * -10  # Penalize heavily
            terminated = np.ones((env.num_envs,), dtype=bool)  # Terminate the episode
            truncated = np.zeros((env.num_envs,), dtype=bool)
            info = {
                'is_success': np.zeros((env.num_envs,), dtype=bool),
                '_is_success': np.zeros((env.num_envs,), dtype=bool),
            }
